    ...
    <div className="space-y-6">
             ...
             <tbody className="divide-y divide-gray-100">
               {filtered.map((page: any) => (
                 <tr key={page.id} className="hover:bg-gray-50/50 transition-colors">
                   ...
                   <td className="p-4">
                     <span className="font-mono text-xs text-black bg-gray-50 border border-gray-100 px-2 py-1 rounded">/p/{page.pageKey}</span>
                   </td>
                   ...
                   <td className="p-4 text-right">
                     {['super_admin', 'editor', 'admin'].includes(role || '') && (
                        <div className="flex justify-end gap-2">
                          ...
                          <button onClick={() => handleDelete(page.id)} className="p-2 text-gray-400 hover:text-gray-600 bg-white rounded-lg border border-gray-200"><Trash2 className="w-4 h-4"/></button>
                        </div>
                     )}
                   </td>
                 </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-none w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col p-6 space-y-4 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="Ex: Mentions Légales" className="w-full border border-gray-200 rounded-none px-4 py-3 font-medium focus:border-black focus:ring-1 focus:ring-[#DA291C] outline-none transition-all"/>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Clé URL (slug)</label>
                  <input required type="text" value={formData.pageKey} onChange={e => setFormData({...formData, pageKey: e.target.value})} placeholder="Ex: mentions-legales" className="w-full border border-gray-200 rounded-none px-4 py-3 font-medium focus:border-black focus:ring-1 focus:ring-[#DA291C] outline-none transition-all"/>
                </div>
              </div>
              <div className="flex-1 flex flex-col">
                <label className="block text-sm font-bold text-gray-700 mb-1">Contenu (Markdown ou HTML simple)</label>
                <textarea required value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})} className="flex-1 min-h-[300px] w-full border border-gray-200 rounded-none px-4 py-3 font-mono text-sm focus:border-black focus:ring-1 focus:ring-[#DA291C] outline-none transition-all resize-none"></textarea>
              </div>
